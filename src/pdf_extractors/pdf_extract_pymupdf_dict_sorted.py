...
def extract_page_sorted(page, join_lines=True):
    """
    Extracts text from a page using PyMuPDF's 'dict' mode with sorting enabled.
    Uses page.get_text("dict", sort=True) to improve reading order.
    Reading order is preserved and sorted as provided by the PDF.
    """
...
def create_debug_pdf(page, blocks, output_path):
    ...
    color = "blue"  # Use blue for sorted dict method
...
def extract_document_sorted_with_debug(filepath, debug_output=None):
    """
    Extract text from document with optional debug PDF output showing block boundaries.
    Uses sorted dict mode for improved reading order.
    """
    doc = pymupdf.open(filepath)
    all_paragraphs = []

    for page_num, page in enumerate(doc, start=1):
        paras = extract_page_sorted(page)
        for p in paras:
...
def extract_document_sorted(filepath):
    doc = pymupdf.open(filepath)
    all_paragraphs = []
...
if __name__ == "__main__":
    ...
    # Build paths relative to project root (corrected path structure)
    pdf_file = script_dir.parent.parent / "data" / "raw" / "ch1_ch14_Brain_and_behavior.pdf"
    debug_output_dir = script_dir.parent.parent / "data" / "debug" / "pdf_extract_pymupdf_dict_sorted"
    debug_output_file = debug_output_dir / "pdf_extract_pymupdf_dict_sorted"
